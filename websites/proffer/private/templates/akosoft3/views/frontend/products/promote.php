<div id="promote_product_box" class="box primary">
	...
	<div class="content">
		...
		<div class="no-promotion promotion_type">
			<span><?php echo ___('products.promote.no_promotion.title') ?></span>
			<?php echo HTML::anchor(products::uri($product).'?preview=1', ___('products.promote.no_promotion.btn'), array('class' => 'button')) ?>
		</div>
		...
		<div class="promotion_type">
			...
			<img src="<?php echo URL::site('/media/products/img/promotion/'.Model_Product::DISTINCTION_PREMIUM.'.png') ?>" class="show" />
						...
						<?php else: ?>
							...
							<div class="payment_method_group">
								<div class="controls">
									...
									<label>
										<input class="payment-method promote-offer" type="radio" value="<?php echo $provider_name ?>" name="payment_method" /> 
										<?php echo ___($provider->get_label()).' - <span class="price">'.$payment_module->show_price($provider, Model_Product::DISTINCTION_PREMIUM).'</span>' ?>
									</label>
									<?php endforeach ?>
								</div>
							</div>
