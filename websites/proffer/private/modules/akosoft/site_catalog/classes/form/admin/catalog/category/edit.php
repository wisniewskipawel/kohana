...
*/
class Form_Admin_Catalog_Category_Edit extends Bform_Form {
		...
		if($category->category_level == 2)
		{
			...
			if($icon_path)
			{
				$this->general->add_html(
					...
					HTML::anchor(
						'admin/catalog/categories/delete_icon/'.$category->pk(),
						$this->trans('catalog.admin.forms.category.delete_icon')
					)
				);
			}

			$this->general->add_input_file('icon', array('label' => $this->trans('catalog.admin.forms.category.icon'), 'required' => FALSE))
