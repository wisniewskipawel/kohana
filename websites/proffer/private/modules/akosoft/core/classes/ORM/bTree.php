...
/**
 * Based on Maciej Kwiatkowski's orm tree
 * 
 * Bug fixes, more customisable retrieving tree and many, many more!
 * 
 * @author Michał Baniowski <baniczek@gmail.com>
 * @author Maciej Kwiatkowski
 * @author AkoSoft Team <biuro@akosoft.pl>
 */
class ORM_bTree extends ORM {
	...
	protected function create_space($start, $size = 2)
	{
		DB::update($this->_table_name)
			->set(array($this->_left_column => DB::expr($this->_db->quote_column($this->_left_column).' + '.$size)))
			->where($this->_left_column, '>=', $start)
			->where($this->_scope_column, '=', $this->scope())
			->execute($this->_db);

		DB::update($this->_table_name)
			->set(array($this->_right_column => DB::expr($this->_db->quote_column($this->_right_column).' + '.$size)))
	...
	{
		$this->where($this->_parent_column, 'IN', DB::select('parent.category_id')
				->from(array($this->table_name(), 'node'))
