...
<div class="toggle_visability_box <?php echo !$filters_sorters['bar'] ? "collapsed" : "expanded" ?>">
	...
	<div class="toggle_visability_target"<?php if(!$filters_sorters['bar']) echo ' style="display: none;"' ?>>
		<div class="filter_entries">
			<form action="" method="get" class="sort auto_submit_form">
				...
				<fieldset class="filters">
					...
					<?php if(in_array($name, array('category_filters', 'index_filters'))): ?>
					<div class="control-group">
						<label><?php echo ___('products.forms.product_manufacturer') ?>:</label>
						<?php 
						echo Form::input(
							'manufacturer', 
							Arr::get($filters, 'manufacturer', NULL), 
							array('class' => 'sort clearable custom')
						); ?>
					</div>
					
					<div class="control-group">
						<label><?php echo ___('products.forms.product_type') ?>:</label>
						<?php 
						$types = Products::types();
						echo Form::select('type', Arr::unshift($types, NULL, ___('select.choose')), Arr::get($filters, 'type', NULL), array('class' => 'sort clearable custom')) ?>
					</div>
				...
				<fieldset class="localization">
					...
					<?php if (Kohana::$config->load('modules.site_products.provinces_enabled')): ?>
		...
		<div class="sorting">
			...
			<div class="sort_type">
				<label><?php echo ___('products.sort.closet') ?></label>
			...
			<?php if(!empty($filters['sort_by']) && $filters['sort_by'] !== TRUE && in_array($filters['sort_by'], array('date_added', 'price'))): ?>
