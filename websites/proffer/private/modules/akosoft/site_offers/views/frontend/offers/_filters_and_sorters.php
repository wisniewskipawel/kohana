	...
	<div class="toggle_visability_target"<?php if(!$filters_sorters['bar']) echo ' style="display: none;"' ?>>
		<div class="filter_entries">
			<form action="" method="get" class="sort auto_submit_form">
				...
				<?php if(in_array($name, array('category_filters', 'index_filters', 'my_filters'))): ?>
				<fieldset class="filters">
					<legend><?php echo ___('filters.title') ?>:</legend>
					<?php if($name == 'category_filters' OR $name == 'index_filters'): ?>

					<?php if (Kohana::$config->load('modules.site_offers.settings.provinces_enabled')): ?>
					<div class="control-group">
						<label><?php echo ___('province') ?>:</label>
						<?php echo Form::select(
							'province', 
							Arr::unshift($provinces, NULL, '---'), 
							Arr::get($filters, 'province', NULL), 
					...
					<div class="control-group">
						...
						<?php 
						$counties = !empty($filters['province']) ? 
							Regions::counties($filters['province']) : array();
					...
					<?php if ($show_clear_link): ?>
					<div class="control-group clear_link_group">
						<a href="<?php echo URL::site($current_uri).URL::query(array(
							'from' => NULL, 
							'province' => NULL, 
							'city' => NULL,
						)) ?>" class="clear_btn"><?php echo ___('filters.clear') ?></a>
					</div>
					<?php endif ?>
		...
		<div class="sorting">
			...
			<div class="sort_type">
				<label><?php echo ___('offers.sort.closet') ?></label>
			...
			<div class="sort_type">
				<label><?php echo ___('offers.sort.popular') ?></label>
				<a class="asc sort <?php if ($filters['sort_by'] === "popular" AND $filters['sort_direction'] == 'asc'): ?> active <?php endif ?>" href="<?php echo URL::site($current_uri).URL::query(array('sort_by' => 'popular', 'sort_direction' => 'asc')) ?>"></a>
				<a class="desc sort <?php if ($filters['sort_by'] === "popular" AND $filters['sort_direction'] == 'desc'): ?> active <?php endif ?>" href="<?php echo URL::site($current_uri).URL::query(array('sort_by' => 'popular', 'sort_direction' => 'desc')) ?>"></a>
			</div>
