...
abstract class Captcha
{
	...
	public function check($response, $valid_answer)
	{
		return (sha1(strtoupper($response)) === $valid_answer);
	}
	...
	public static function valid($response)
	{
		...
		// Challenge result
		$result = (bool) $instance->check($response, Session::instance()->get('captcha_response'));
		...
		if ($counted !== TRUE)
		{
			...
			if ($result === TRUE)
			{
				$instance->valid_count(Session::instance()->get('captcha_valid_count') + 1);
			}
			// Invalid response
			else
			{
				$instance->invalid_count(Session::instance()->get('captcha_invalid_count') + 1);
			}
		}
	...
	public function image_render($html)
	{
		// Output html element
		if ($html === TRUE)
			return '<img src="'.URL::site('captcha/'.Captcha::$config['group'], 'http').'" width="'.Captcha::$config['width'].'" height="'.Captcha::$config['height'].'" alt="Captcha" class="captcha" />';

		// Send the correct HTTP header
		$this->request_response->headers('Content-Type', 'image/'.$this->image_type);
		$this->request_response->headers('Cache-Control', 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0');
		$this->request_response->headers('Pragma', 'no-cache');
		$this->request_response->headers('Connection', 'close');
	...
	public function set_request_response(Response $response)
	{
