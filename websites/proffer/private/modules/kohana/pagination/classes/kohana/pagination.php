...
class Kohana_Pagination {
	...
	public function url($page = 1)
	{
		...
		{
			case 'query_string':
				return URL::site(Request::current()->uri(), FALSE, TRUE, FALSE).URL::query(array($this->config['current_page']['key'] => $page));

			case 'route':
				return URL::site(Request::current()->uri(array($this->config['current_page']['key'] => $page)), FALSE, TRUE, FALSE).URL::query();
		}
