...
<div id="resume-manager-candidate-dashboard">
	<div class="alert alert-info">
		<p><?php echo _n( 'Your resume can be viewed, edited or removed below.', 'Your resume(s) can be viewed, edited or removed below.', resume_manager_count_user_resumes(), 'petsitter' ); ?></p>
	</div>
	<div class="table-responsive">
		<table class="resume-manager-resumes table table-bordered table-striped">
			...
			<tbody>
				<?php if ( ! $resumes ) : ?>
					<tr>
						<td colspan="<?php echo sizeof( $candidate_dashboard_columns ); ?>"><?php _e( 'You do not have any active resume listings.', 'petsitter' ); ?></td>
					</tr>
				<?php else : ?>
								...
								<td class="<?php echo esc_attr( $key ); ?>">
										...
										<ul class="candidate-dashboard-actions">
											<?php
												...
												switch ( $resume->post_status ) {
													case 'publish' :
														$actions['edit'] = array( 'label' => __( 'Edit', 'petsitter' ), 'nonce' => false );
														$actions['hide'] = array( 'label' => __( 'Hide', 'petsitter' ), 'nonce' => true );
													break;
													case 'hidden' :
														$actions['edit'] = array( 'label' => __( 'Edit', 'petsitter' ), 'nonce' => false );
														$actions['publish'] = array( 'label' => __( 'Publish', 'petsitter' ), 'nonce' => true );
													break;
													case 'expired' :
														if ( get_option( 'resume_manager_submit_resume_form_page_id' ) ) {
															$actions['relist'] = array( 'label' => __( 'Relist', 'petsitter' ), 'nonce' => true );
														}
													break;
												}

												$actions['delete'] = array( 'label' => __( 'Delete', 'petsitter' ), 'nonce' => true );
									...
									<?php elseif ( 'date' === $key ) : ?>
										...
										if ( ! empty( $resume->_resume_expires ) && strtotime( $resume->_resume_expires ) > current_time( 'timestamp' ) ) {
											printf( __( 'Expires %s', 'petsitter' ), date_i18n( get_option( 'date_format' ), strtotime( $resume->_resume_expires ) ) );
										} else {
				...
				<tfoot>
					<tr>
						<td colspan="<?php echo sizeof( $candidate_dashboard_columns ); ?>">
							<a href="<?php echo esc_url( get_permalink( $submit_resume_form_page_id ) ); ?>" class="btn btn-secondary"><?php _e( 'Add Resume', 'petsitter' ); ?></a>
						</td>
					</tr>
				</tfoot>
			<?php endif; ?>
